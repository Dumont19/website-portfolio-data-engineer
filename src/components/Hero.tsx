...
function LoadingText() {
  return (
    <span className="text-[10px] md:text-xs font-mono text-neutral-500 uppercase tracking-[0.4em]">
      Loading Data
      <span className="inline-flex gap-[2px] ml-1">
        <span className="animate-[bounce_1s_ease-in-out_infinite_0ms]">.</span>
        <span className="animate-[bounce_1s_ease-in-out_infinite_150ms]">.</span>
        <span className="animate-[bounce_1s_ease-in-out_infinite_300ms]">.</span>
      </span>
    </span>
  );
}
      ...
      <AnimatePresence>
        {isIntro && (
            <motion.div 
                ...
                className="fixed inset-0 z-[200] flex flex-col items-center justify-end pb-16 md:pb-24 lg:pb-16 pointer-events-none"
            >
                <LoadingText />
            </motion.div>
        )}
